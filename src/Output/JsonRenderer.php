    ...
    private function findingPlan(FindingPlan $fp, Plan $plan): array
    {
        ...
        return [
            ...
            'advisories' => array_map(static fn (Finding $x): array => [
                ...
                'epss' => $x->advisory->epss,
                'epss_percentile' => $x->advisory->epssPercentile,
                'kev_added' => $x->advisory->kevAdded?->format('Y-m-d'),
            ], $fp->allFindings()),
            'abandoned' => array_map(static fn (string $name): array => ['package' => $name, 'replacement' => $f->abandoned[$name]], array_keys($f->abandoned)),

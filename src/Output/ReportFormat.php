...
enum ReportFormat: string
{
    ...
    /**
     * Accepts "html", "report.html", "report.sarif", "results.sarif.json", "json:build/report.json"
     * and returns [format, path|null].
     *
     * @return array{self, string|null}
     */
    public static function parseOutputSpec(string $spec): array
    {
        if (preg_match('{^(text|html|json|sarif):(.+)$}i', $spec, $m) === 1) {
            return [self::from(strtolower($m[1])), $m[2]];
        }
        if (str_ends_with(strtolower($spec), '.sarif.json')) {
        ...
        if ($format === null) {
            throw new \InvalidArgumentException(sprintf('Cannot infer a report format from "%s"; use a .html/.json/.sarif/.txt extension or prefix with html:, json:, sarif: or text:.', $spec));
        }

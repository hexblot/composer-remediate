...
# It is set on the remote rather than passed to the push, so that fetches are authenticated too and so
# that --force-with-lease still has a remote-tracking ref to lease against, which a push straight to a
# URL does not.
...
git fetch -q origin "+refs/heads/$BRANCH:refs/remotes/origin/$BRANCH" || true

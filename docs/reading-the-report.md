...
| `The analysed project's composer.json suppresses advisories …` | The repository's own `config.audit.ignore` or `config.policy.advisories` left findings out. Intended for a project you own; for one you are scanning, the entries are named so you can review them. |
| `The analysed project's composer.json changes how TLS certificates are verified …` | The repository supplied `config.cafile`, `capath` or `disable-tls`, which Composer used when fetching the advisory database. |

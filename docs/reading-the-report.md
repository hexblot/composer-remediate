...
| `The analysed project's composer.json suppresses advisories …` | The repository's own `config.audit.ignore` or `config.policy.advisories` left findings out. Intended for a project you own; for one you are scanning, the entries are named so you can review them, and `--no-project-ignores` drops them. |
| `The analysed project's composer.json changes how TLS certificates are verified …` | The repository supplied `config.cafile`, `capath` or `disable-tls`. They did not apply: the advisory database was fetched with your own TLS configuration. |
| `The analysed project's composer.json sets config.cache-dir …` | The database is kept under the operator's cache directory instead, so a repository cannot pre-fill the path the scan reads. |
